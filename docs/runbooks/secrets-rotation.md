...
production use the Render env group `kritifin-secrets` and Vercel project
settings. CI scans every push with gitleaks.

## Inventory

| Secret | Where used | Rotation | Cadence |
|--------|-----------|----------|---------|
| `DATABASE_URL` (kritifin_app password) | Render api+worker | `ALTER ROLE kritifin_app PASSWORD '...'` then update Render, redeploy | 90 days / on suspicion |
| `DATABASE_ADMIN_URL` (postgres password) | Render preDeploy, break-glass | Supabase Dashboard → Database → reset password | 90 days |
...
2. Update the Render env group / Vercel env; trigger redeploy.
